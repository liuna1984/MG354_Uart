...
#define CONSTRUCT_PARSE_PARAM_MEMORY_POOL(buf, size) unsigned char buf##_memory[size];\
                                                  ParseParam buf = {0,PARSE_INITIAL,0,0,0,(size),0,0,{0},buf##_memory};
...
int16_t GetShortBigEnd(uint8_t* pBuffer, int32_t* nCounter);
uint16_t GetUShortBigEnd(uint8_t* pBuffer, int32_t* nCounter);
int32_t GetLongBigEnd(uint8_t* pBuffer, int32_t* nCounter);
uint32_t GetULongBigEnd(uint8_t* pBuffer, int32_t* nCounter);
